...
- Moved lock state management into the Electron main process with forced lock-route enforcement while notes are locked
...
- Added a Note font setting with a live preview for changing note title and content fonts
- Added a Note size setting for changing note card dimensions with four predefined sizes
- Added drag-and-drop note reordering with a persisted custom order
...
- Added optional note title section
- Added a Settings window for Electron and an embedded Settings dialog for the web app
...
- Added a dedicated License window opened from the Help menu
